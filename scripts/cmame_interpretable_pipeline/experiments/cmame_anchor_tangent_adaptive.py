...
def _build_tangent_materials(
    *,
    ...
    selected = tuple(range(len(reduced.COEFF_NAMES))) if coefficient_indices is None else coefficient_indices
    for q in selected:
        name = reduced.COEFF_NAMES[q]
            ...
            while abs(used_step) >= float(min_rel_step) * float(scales[q]):
                ...
                try:
                    mat = material_from_gamma(
                        trial,
                        material_id=100000 + 1000 * anchor_id + q,
            ...
            while abs(used_step) >= float(min_rel_step) * float(scales[q]):
                for sign_label, sign in [("plus", 1), ("minus", -1)]:
                    ...
                    try:
                        mat = material_from_gamma(
                            trial,
                            material_id=100000 + 1000 * anchor_id + 2 * q + (0 if sign > 0 else 1),
...
def append_raw_normalized(
    existing_basis: list[np.ndarray],
    new_fields: list[np.ndarray],
) -> list[np.ndarray]:
    """Append an independent first-anchor block without a voxelwise QR transform.

    The six homogeneous affine tangent dependency is removed before this call.
    Per-row scaling improves reduced conditioning while preserving the Ritz span.
    """
    if existing_basis:
        raise ValueError("raw-normalized construction is restricted to the first anchor")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--monitor-truth-source", type=Path, default=None)
    parser.add_argument("--final-truth-source", type=Path, default=None)
    ...
    parser.add_argument(
        "--omit-redundant-tangent",
        action=argparse.BooleanOptionalAction,
        default=True,
        help=(
            "Skip the affine tangent with largest |gamma_q|. Homogeneity gives "
            "sum_q gamma_q dX/dgamma_q=0, so the omitted six-load block does not "
            "change the tangent span."
        ),
    )
    ...
    parser.add_argument(
        "--ritz-compute-dtype", choices=("float32", "float64"), default="float32",
        help="Precision of large spatial contractions; reduced accumulators remain float64.",
    )
    parser.add_argument(
        "--ritz-compiler", choices=("factorized", "conventional"), default="factorized",
        help="Use the paper's exact constitutive-factor compiler or the conventional baseline.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--basis-construction",
        choices=("orthonormal", "raw-normalized"),
        default="orthonormal",
    )
...
def main() -> int:
    ...
    affine = reduced.affine_stress_batch_factory(
        operator_phase,
        operator_ori,
        gathered_factor_ritz=(
            str(args.ritz_compiler) == "factorized" and bool(args.gathered_factor_ritz)
        ),
    )
    ...
    base_monitor_truth_csv = Path(args.monitor_truth_source).resolve() if args.monitor_truth_source else (
        out_root / "runs"
        / f"{BASE_RUN_STEM}_geometry_{int(args.geometry_id):02d}"
        / "monitor_truth_results.csv"
    )
    base_final_truth_csv = Path(args.final_truth_source).resolve() if args.final_truth_source else (
        out_root / "runs"
        / f"{BASE_RUN_STEM}_geometry_{int(args.geometry_id):02d}"
        / "final_validation_truth_results.csv"
    )
    monitor_truth_csv = (
        base_monitor_truth_csv if args.monitor_truth_source else run_dir / "monitor_truth_results.csv"
    )
    final_truth_csv = (
        base_final_truth_csv if args.final_truth_source else run_dir / "final_validation_truth_results.csv"
    )
    ...
    for anchor_idx in range(max_anchors):
        ...
        omitted_tangent = (
            int(np.argmax(np.abs(anchor_gamma)))
            if bool(args.omit_redundant_tangent)
            else None
        )
        sensitivity_indices = tuple(
            q for q in range(len(reduced.COEFF_NAMES)) if q != omitted_tangent
        )
        print(
        ...
        if str(args.tangent_method) == "sensitivity":
            step_df = pd.DataFrame([
                {
                    ...
                    "fd_mode": "",
                    "signed_step": 0.0,
                    "solved": bool(q in sensitivity_indices),
                    "omitted_by_homogeneity": bool(q == omitted_tangent),
                }
            ...
            perturbed_materials, step_df = _build_tangent_materials(
                ...
                anchor_id=anchor_idx,
                fd_mode=fd_mode,
                coefficient_indices=sensitivity_indices,
            )
            step_df.to_csv(
            ...
            for row in ([] if fd_mode == "forward" else step_df.to_dict(orient="records")):
                ...
                plus_id = 100000 + 1000 * anchor_idx + 2 * q
                minus_id = 100000 + 1000 * anchor_idx + 2 * q + 1
        ...
        if not basis_streamed:
            ...
            if str(args.basis_construction) == "raw-normalized":
                if int(args.max_anchors) != 1:
                    raise ValueError("raw-normalized basis construction currently requires max-anchors=1")
                appended_fields = append_raw_normalized(basis_fields, tangent_blocks)
            else:
        ...
        if operators is None:
            ...
            operators, assembly_meta = common.update_reduced_operators(
                ...
                contraction_compute_dtype=str(args.ritz_compute_dtype),
                gram_compute_dtype="float64",
                preserve_raw_coordinates=str(args.ritz_compiler) == "factorized",
                factorized_ritz=str(args.ritz_compiler) == "factorized",
            )
        else:
        ...
        if str(args.basis_construction) == "raw-normalized":
            stabilized, stabilization_meta = reduced._reference_energy_qr_recompile(
                raw_Kq=operators["raw_Kq"],
                raw_Bq=operators["raw_Bq"],
                Dq=operators["Dq"],
                reference_coefficients=anchor_gamma,
            )
            operators.update(stabilized)
            assembly_meta.update({
                f"post_{key}": value for key, value in stabilization_meta.items()
            })
            assembly_wall_s = float(time.perf_counter() - assembly_started)
