...
def _samples(kind: str, count: int, seed: int) -> np.ndarray:
    ...
    if kind == "local_truncated_normal":
        center = 0.5 * (lower + upper)
        sigma = (upper - lower) / 6.0
        samples = np.empty((int(count), len(PARAMETER_NAMES)), dtype=np.float64)
        filled = 0
        while filled < int(count):
            batch_size = max(int(count) - filled, 1024)
            proposals = rng.normal(center, sigma, size=(batch_size, len(PARAMETER_NAMES)))
            accepted = proposals[np.all((proposals >= lower) & (proposals <= upper), axis=1)]
