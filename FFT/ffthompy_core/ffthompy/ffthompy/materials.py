...
class Material():
    ...
    def __init__(self, material_conf):
        ...
        if (
            'fun' in self.conf
            or 'Cfield_path' in self.conf
            or 'Cfield' in self.conf
            or ('Cfield_index' in self.conf and 'Cfield_table' in self.conf)
        ):
            pass
        ...
        if order in [None, 'exact']: # inclusion-based composite
            ...
            for m,n in itertools.product(*(list(range(d)) for d in vals.shape)):
                ...
                if np.allclose(P, Nbar):
                    ...
                    hAM0per = np.tile(hAM0, 2*np.array(factor, dtype=int)+1)
    ...
    def get_A_GaNi(self, N, primaldual='primal', tensor=True, fft_form='c'):
        """
        Returns stiffness matrix for a scheme with trapezoidal quadrature rule.
        """
        use_fast_cfield = bool(self.conf.get('use_cfield_fast_path', True))
        if use_fast_cfield and 'Cfield_index' in self.conf and 'Cfield_table' in self.conf:
            if primaldual != 'primal':
                raise NotImplementedError("Cfield indexado solo soporta formulacion primal.")
            index = self.conf['Cfield_index']
            A = Tensor(
                name='A_GaNi',
                val=index[None, ...],
                order=1,
            ...
            A.material_table = self.conf['Cfield_table']
            return A
        if use_fast_cfield and ('Cfield' in self.conf or 'Cfield_path' in self.conf):
            cfield_origin = str(self.conf.get('cfield_origin', 'centered')).strip().lower()
            origin = 0 if cfield_origin in {'0', 'zero', 'native', 'grid'} else 'c'
            if 'Cfield_path' in self.conf:
                A_val = np.load(self.conf['Cfield_path'], mmap_mode='r')
            else:
                A_val = self.conf['Cfield']

            shp = A_val.shape[:A_val.ndim-len(N)]
            if tensor:
                cfield_storage = str(self.conf.get('cfield_storage', 'full')).strip().lower()
                if cfield_storage in {'sym21', 'symmetric21', 'packed21'}:
                    multype = 'sym21'
                else:
                    multypes={'2':21, '4': 42}
                    multype = multypes['{}'.format(len(shp))]
                A = Tensor(name='A_GaNi', val=A_val, order=len(shp), N=N,
                           Y=self.Y, multype=multype,
                           Fourier=False, origin=origin, fft_form=fft_form)
            else:
                A = Matrix(name='A_GaNi', val=A_val, Fourier=False)
            if primaldual == 'dual':
                A = A.inv()
            return A if origin == 0 else A.shift()

        coord = Grid.get_coordinates(N, self.Y)
        A = self.evaluate(coord, tensor=tensor, fft_form=fft_form)
        if primaldual == 'dual':
            A = A.inv()
        return A.shift()
    ...
    def evaluate(self, coord, tensor=True, fft_form='c'):
        """
        ...
        if 'fun' in self.conf:
            ...
            A_val = fun(coord)
            shp=A_val.shape[:A_val.ndim-len(N)]
        elif 'Cfield_path' in self.conf:
            A_val = np.load(self.conf['Cfield_path'], mmap_mode='r')
            shp=A_val.shape[:A_val.ndim-len(N)]
        elif 'Cfield' in self.conf:
            A_val = self.conf['Cfield']
            shp=A_val.shape[:A_val.ndim-len(N)]
        else:
        ...
        if tensor:
            ...
            return Tensor(name='A_GaNi', val=A_val, order=len(shp), N=coord[0].shape,
                          ...
                          Fourier=False, origin='c', fft_form=fft_form)
        else:
    ...
    def get_topologies(self, coord):
        ...
        N = np.array(coord.shape[1:], dtype=int)
        ...
        for dd in np.arange(dim):
            ...
            cop[dd] = np.tile(coord[dd], 3*np.ones(dim, dtype=int)) + Ymat
